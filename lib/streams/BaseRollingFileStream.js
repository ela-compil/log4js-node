...
function BaseRollingFileStream(filename, options) {
  ...
  if (this.currentSize == 0 && this.options.template) {
    this.write(this.options.template, "utf8");
  }
}
...
BaseRollingFileStream.prototype._write = function(chunk, encoding, callback) {
  ...
  function writeTemplateAndChunk() {
    that.currentSize += that.options.template.length;
    that.theStream.write(that.options.template, encoding, writeTheChunk);
  }
  ...
  if (this.shouldRoll()) {
    ...
    if (this.options.template) {
      this.roll(this.filename, writeTemplateAndChunk);
    }
    else {
