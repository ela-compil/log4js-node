 ...
 */
function fileAppender (file, layout, logSize, numBackups, compress, timezoneOffset, layoutTemplate) {
  var bytesWritten = 0;
  ...
  function openTheStream(file, fileSize, numFiles) {
    ...
    if (fileSize) {
      stream = new streams.RollingFileStream(
        ...
        fileSize,
        numFiles,
        { "compress": compress, template: layoutTemplate }
      );
    } else {
...
function configure(config, options) {
  ...
  if (config.layout) {
    ...
    layoutTemplate = config.layout.template;
  }
